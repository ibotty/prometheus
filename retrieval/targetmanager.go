...
// populateLabels builds a target from the given label set and scrape configuration.
// Returns a nil target if the target is dropped due during relabeling.
func populateLabels(lset model.LabelSet, cfg *config.ScrapeConfig) (res, orig model.LabelSet, err error) {
	...
	// hasPort checks whether s is an IPv6 or IPv4 address that contains a port.
	// We don't have to catch whether they are valid addresses, just whether they
	// have a port if they are valid.
	hasPort := func(s string) bool {
		return strings.HasPrefix(s, "[") && !strings.HasSuffix(s, "]") ||
			strings.Count(s, ".") == 3 && strings.Contains(s, ":")
	}
	// If it's an address with no trailing port, infer it based on the used scheme.
	if addr := string(lset[model.AddressLabel]); !hasPort(addr) {
